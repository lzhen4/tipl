...
var _clockTicks;
var _sineSignal;
function generateWave(frequency, duration, amplitude, sampling = 48000) {
    ...
    for (var i, _pj_c = 0, _pj_a = _clockTicks, _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        i = _pj_a[_pj_c];
        _sineSignal.push((amplitude * Math.sin(((((2 * Math.PI) * frequency) / sampling) * i))));
    }
    return _sineSignal;
}
...
var _silent;
var _soundBin;
function generateSoundbin(frequency, duration, interval, amplitude, sampling = 48000, rampSize = 0.005) {
    var _silent, _sineWave, _soundBin;
    _sineWave = generateWave(frequency, duration, amplitude, sampling);
...
var _sequence;
function generateAuditorySequence(nbins, initialFrequency, deltaGauss, toneDuration, interToneInterval, amplitude, sampling = 48000, rampSize = 0.005, std = 1.0) {
    ...
    for (var i, _pj_c = 0, _pj_a = util.range(nbins), _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        ...
        abin = generateSoundbin(selected, toneDuration, interToneInterval, amplitude, sampling, rampSize);
...
async function experimentInit() {
  ...
  acousticVolume = Number.parseFloat(acousticSetting[3]);
...
function prepareRoutineBegin(snapshot) {
  return async function () {
    ...
    for (var beepDir, _pj_c = 0, _pj_a = beepDirList, _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        beepDir = _pj_a[_pj_c];
        beep = generateAuditorySequence(nbins, initialFrequency, (deltaGauss * beepDir), toneDuration, interToneInterval, acousticVolume, 48000, rampSize, stdevTone);
        beeps.push(beep);
    }
