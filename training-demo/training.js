...
async function experimentInit() {
  ...
  rdkDirectionText = new visual.TextStim({
    win: psychoJS.window,
    name: 'rdkDirectionText',
    text: '',
    font: 'Arial',
    units: undefined, 
  ...
  rdkParamText = new visual.TextStim({
    win: psychoJS.window,
    name: 'rdkParamText',
    text: '',
    font: 'Open Sans',
...
function loop_setLoopBegin(loop_setLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisLoop_set of loop_set) {
...
function prepareRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prepareComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    trialComponents.push(tokenText);
    trialComponents.push(opacity_text);
    trialComponents.push(rdkDirectionText);
    trialComponents.push(rdkParamText);
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *tokenText* updates
    if (t >= 0.0 && tokenText.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      tokenText.tStart = t;  // (not accounting for frame time here)
    ...
    if (tokenText.status === PsychoJS.Status.STARTED && t >= frameRemains) {
    ...
    if (t >= 0.0 && rdkDirectionText.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      rdkDirectionText.tStart = t;  // (not accounting for frame time here)
    ...
    frameRemains = 0.0 + 5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (rdkDirectionText.status === PsychoJS.Status.STARTED && t >= frameRemains) {
    ...
    if (t >= 0.0 && rdkParamText.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      rdkParamText.tStart = t;  // (not accounting for frame time here)
    ...
    frameRemains = 0.0 + 5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (rdkParamText.status === PsychoJS.Status.STARTED && t >= frameRemains) {
